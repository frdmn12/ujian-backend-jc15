...
router.post("/login", (req, res) => {
  ...
  let sql = `
    SELECT 
        ...
        role,
        status
    FROM users WHERE users = '${username}' AND password = '${hashPassword(
    password
  )}'`;
  db.query(sql, (err, data) => {
    if (err) {
...
router.post("/register", async (req, res) => {
  ...
  try {
    const insert = await query(
      `INSERT INTO users (username, email, password, role, status) VALUES ('${username}', '${email}', '${password}',2, 0)`
    );
    const mailOptions = {
      from: "Admin <bayuferdiman1@gmail.com>",
      to: email,
    ...
    const select = await query(
      ...
      email, 
      role,
      status FROM users WHERE id = ${insert.insertId}`
    );
